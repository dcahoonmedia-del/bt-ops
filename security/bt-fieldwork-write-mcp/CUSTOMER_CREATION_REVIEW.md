...
`primary_email` is one customer PATCH, `customer[invoice_email]` only, after the customer POST and Main Location step. POST support is not claimed. On 2026-09-25 at 17:02 Eastern that PATCH returned HTTP 200 and the same-as-billing Main Location email changed with it. Propagation is not claimed for other location setups. Notice delivery was not audited. A differing `location_email` is preserved by the location email PATCH after that customer PATCH. No contact is created for the primary email. GET location omits `reminders_type`, so readback stays `unverified` with a manual office check. A present value other than `0` is a mismatch. Customer-creation notice behavior is unknown. UI form names are not API routes.
