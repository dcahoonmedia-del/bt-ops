...
3. Josh (`Joshua1740@icloud.com`) uses the same `FW_WRITE_PERMITTED_USERS` list as Daniel, not a read-only role. Production today lists only `daniel@btpestcontrol.com`. After the UI approval, set `FW_WRITE_PERMITTED_USERS=daniel@btpestcontrol.com,joshua1740@icloud.com`. Do not add a subject-map entry until a verified Auth0 subject is read. Do not infer that subject from the email. No auth code change is in this checkout.
