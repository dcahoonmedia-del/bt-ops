...
7. Live `GET /profile` roles are the permission names `schedule`, `customers`, `invoicing`, `reporting`, `agreements`, `tasks`, and `work_orders`. `readonly` wins. Any other known permission name is writer. Empty, malformed, and all-unknown role lists stay `api_role_unverified`. Each propose, execute, inspect, and report refreshes that snapshot once and reuses it for that response.
