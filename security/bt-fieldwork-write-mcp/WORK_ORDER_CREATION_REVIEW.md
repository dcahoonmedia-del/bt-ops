...
`POST /work_orders` sends root `service_appointment` with customer, location, `repeat_type` none, integer `repeat_period`, one occurrence (`service_route_ids`, `starts_at` `YYYY-MM-DD`, optional duration, instructions, production_value), and the catalog line. `started_at_time`, `finished_at_time`, `private_notes`, `status`, and `use_time_window` are `unknown_field` with that field list. Promised-window enforcement is false.
