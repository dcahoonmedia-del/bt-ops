...
from .allowlist import GATE_AUTH_UNRESOLVED, GATE_MAPPING_UNVERIFIED, GATE_SCHEMA_UNVERIFIED, is_lead_status
...
class HttpTransport:
    """Live urllib transport. Tests must not construct this against HQ."""

    def __init__(self, api_key: InMemoryApiKey, api_base: str = API_BASE) -> None:
        self._key = api_key
    ...
    def request(self, method: str, path: str, body: dict[str, Any] | None = None) -> tuple[int, Any]:
        ...
        req.add_header("Authorization", f"Token token={self._key.get()}")
        if data is not None:
        ...
        try:
            ...
            if sent and status >= 500:
                raise AmbiguousWriteError(f"remote_{status}") from exc
            try:
            ...
            if sent:
                raise AmbiguousWriteError("transport_drop_after_send") from exc
            raise
        if not raw:
            return status, None
        try:
            ...
            if sent:
                raise AmbiguousWriteError("unreadable_write_response") from exc
            raise
...
class FakeTransport:
    ...
    def request(self, method: str, path: str, body: dict[str, Any] | None = None) -> tuple[int, Any]:
        ...
        if method == "GET" and (_WORK_ORDER.match(path) or _WORK_ORDER_PLAIN.match(path) or _WORK_ORDER_SEARCH.match(path)):
            ...
            return (200, wo) if wo else (404, None)
        if method in {"POST", "PATCH"} and path.startswith("/work_orders"):
            return 403, {"error": "mapping_unverified_not_called"}
        return 404, None
...
def _assert_typed_path(method: str, path: str) -> None:
    ...
    if method == "PATCH" and _WORK_ORDER.match(path):
        raise GateError(GATE_MAPPING_UNVERIFIED)
