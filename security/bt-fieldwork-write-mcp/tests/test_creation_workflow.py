...
class CreationWorkflowTests(unittest.TestCase):
    ...
    def test_distinct_address_and_explicit_contact(self) -> None:
        ...
        self.assertEqual(blocked["coverage_gap"], ["address", "email"])
        proposed = self.h.service.propose("create_customer", {**payload, "acknowledge_duplicate_coverage": ["address", "email"]}, IDENTITY)
        self.assertTrue(proposed["ok"], proposed)
        ...
        self.assertFalse(any(call["method"] == "POST" for call in self.h.transport.calls if call["body"] and call["body"].get("customer", {}).get("last_name") == "Bare"))
