...
A second internal transport is authorized for `revise_draft` only: `daniel@btpestcontrol.com` → `daniel+lead-desk@btpestcontrol.com`. That path does not require inbound Authentication-Results (self-addressed Gmail omits them). Production fetch must call `users.getProfile` on the live token (not token-file email), require the returned Gmail id to match, prove SENT, and check every raw From/To/Cc/Bcc occurrence. Freshness is Gmail `internalDate` versus processing time, 15 minutes, same length as the contactus Sent/received window; an unchanged draft is not indefinite approval. Header spoofing, Bcc extras, or copied syntax on customer/external mail cannot satisfy it. Automatic discovery is a disabled Daniel-only history poller (messageAdded and labelAdded, SENT + Control label). One combined result is intended for `daniel+lead-desk-results@` through a separate send-only credential. Missing sender config is an activation blocker and does not consume a decision. See `results/plus-control/CONTRACT.md`.
